...
Cheese = Product('Сыр', 50, 59)
Chicken = Product('Курица', 100, 36)
Pineapple = Product('Ананас', 80, 29)
Sauce = Product('Соус', 10, 9)
Dough = Product('Тесто', 50, 39)
...
Hawai = Pizza('Гавайская', [In1, In2, In3, In4, In5])

total_kkal = 0
total_price = 0
for i in range (0, len(Hawai.ingredients)):
    total_kkal += Hawai.ingredients[i].get_kkal()
    total_price += Hawai.ingredients[i].get_price()

print(f'{Hawai.title} ({total_kkal} kkal) - {total_price} руб')